...
aggregate_categories = function(
  ...
  data <- rbind(
    ...
    select(-tractid10) %>%
    mutate(cat = factor(cat, levels = c("Overall", race_cat_plot_order, inc_cat, gent_cat_plot_order))) %>%
    mutate(facet = factor(facet, levels = c("All", "Ethnoracial", "Income", "Gentrification"))) %>%
    filter(!is.na(facet))
